...
export class BlockchainProxy {
   ...
   */
  async claimOwnership(productId: string, ownerInfo: any): Promise<any> {
    try {
   ...
   */
  async generateTransferCode(productId: string, currentOwnerHash: string, securityHash: string): Promise<any> {
    try {
   ...
   */
  async completeTransfer(productId: string, transferCode: string, newOwnerInfo: any): Promise<any> {
    try {
   ...
   */
  async initiateReturn(productId: string, reason: string, ownerHash: string): Promise<any> {
    try {
  ...
  /**
   * Report stolen product (with security verification)
   */
  async reportStolen(productId: string, ownerHash: string, securityHash: string, policeReportId?: string): Promise<any> {
    try {
   ...
   */
  async recoverProduct(productId: string, ownerHash: string, securityHash: string, recoveryDetails?: string): Promise<any> {
    try {
      const response = await this.client.post('/api/supply-chain/ownership/recover', {
        productId,
        ownerHash,
        securityHash,  // Requires security verification
        recoveryDetails: recoveryDetails || 'Product recovered by owner'
      });
      ...
      console.error('Failed to recover product:', error.message);
      throw new Error('Failed to mark product as recovered');
    }
  }
   ...
   */
  async getBirthCertificate(productId: string): Promise<any> {
    try {
   ...
   */
  async getOwnedProducts(ownerHash: string): Promise<any[]> {
    try {
   ...
   */
  private generateOwnerHash(ownerInfo: any): string {
   ...
   * Generate security hash from password and PIN
   */
  private generateSecurityHash(password: string, pin: string): string {
    const crypto = require('crypto');
    if (!password || !pin) {
